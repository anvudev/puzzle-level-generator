...
describe('Level Generator Tests', () => {
  ...
  describe('Color Balance Validation', () => {
    it('should ensure all colors are divisible by 3 - basic case', () => {
      ...
      for (const [color, count] of colorCounts) {
        expect(count % 3).toBe(0, `Color ${color} has ${count} blocks, should be divisible by 3`)
      }
    })
    ...
    it('should handle pipe elements correctly in color balance', () => {
      ...
      expect(pipeCount).toBe(2, 'Should have exactly 2 pipes')
  ...
  describe('Element Count Validation', () => {
    it('should place exact number of requested elements', () => {
      ...
      for (let y = 0; y < level.config.height; y++) {
        for (let x = 0; x < level.config.width; x++) {
          const cell = level.board[y][x]
          if (cell.element) {
            elementCounts.set(cell.element, (elementCounts.get(cell.element) || 0) + 1)
          }
        }
      }
      ...
      expect(elementCounts.get('Pipe')).toBe(3, 'Should have exactly 3 pipes')
      expect(elementCounts.get('Barrel')).toBe(2, 'Should have exactly 2 barrels')
      expect(elementCounts.get('Block Lock')).toBe(1, 'Should have exactly 1 block lock')
      expect(elementCounts.get('Key')).toBe(1, 'Should have exactly 1 key (for the lock)')
    })
    ...
    it('should handle multiple element types correctly', () => {
      ...
      const level = FallbackLevelGenerator.generateLevel(configWithManyElements)
  ...
  describe('Board Connectivity', () => {
    it('should generate connected boards', () => {
      for (let i = 0; i < 5; i++) { // Test multiple generations
        const level = FallbackLevelGenerator.generateLevel(basicConfig)
        const isConnected = LevelGeneratorUtils.isConnected(level.board)
        expect(isConnected).toBe(true, `Board ${i + 1} should be connected`)
      }
    })
  })
    ...
    it('should generate symmetric boards when requested', () => {
      ...
      for (let y = 0; y < level.config.height; y++) {
        for (let x = 0; x < centerX; x++) {
          ...
          if (leftCell.type !== rightCell.type || leftCell.color !== rightCell.color) {
            symmetryViolations++
          }
        }
      }
      
      expect(symmetryViolations).toBe(0, 'Board should be symmetric')
    })
  })
})
